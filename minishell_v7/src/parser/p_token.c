...
/*   Updated: 2024/10/02 17:56:34 by yfontene         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int token_quotes_end(char *str, int i)
{
    ...
    if (!str || i < 0)
    {
        printf("Erro: null string or invalid starting index\n");
        return -1;
    }
    while (str[i])
    {
    ...
    printf("Erro: A matching citation was not found for the character at position %d\n", i);
    return (-1);
}
