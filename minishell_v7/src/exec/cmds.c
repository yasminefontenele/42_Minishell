...
/*   Updated: 2024/10/02 18:00:35 by yfontene         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void command_get_single(t_shell *shell, t_list *comnd)
{
    ...
    if (node->path && access(node->path, X_OK) == 0)
    {
        ...
        else
        {
            ...
            rl_replace_line("", 0); // Limpa a linha de comando
...
void command_get_redir(t_shell *shell, t_list *comnd)
{
    ...
    if (built_check(node)) 
    {
        ...
        //printf("dup2 done: original_stdout = %d, STDOUT_FILENO = %d\n", original_stdout, STDOUT_FILENO);
        //printf("Before command: STDOUT_FILENO = %d\n", STDOUT_FILENO);
        
        if (dup2(original_stdout, STDOUT_FILENO) == -1)
            perror("Error restoring STDOUT after built-in");
        //else
            //printf("STDOUT restored after built-in\n");
